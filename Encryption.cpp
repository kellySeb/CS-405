...
std::string read_file(const std::string& filename)
{
    ...
    if (!file.is_open())
    {
        //return error message if not opened successfully
        std::cerr << "Could not open the file: " << filename << std::endl;
        return "";
    }

    //read entire contents into stringstream buffer
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
    ...
    if (!file.is_open())
    {
        //error message if not successfully opened
        std::cerr << "Could not open the file for writing: " << filename << std::endl;
        return;
    }

    //write in order requested
    file << student_name << '\n';  
    file << key << '\n';           
    file << data;                  
